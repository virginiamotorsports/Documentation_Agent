...
class BoxAgent:
    ...
    def box_ingestion(self): 
        ...
        for file in files:
            ...
            extracted_text_allowed = filter(lambda x: x["representation"] == "extracted_text", representations)
            
            if extracted_text_allowed:
                url_template = box_client.get_extracted_text_representation_metadata(file["id"])["representations"]["entries"][0]["content"]["url_template"]
                url_template = url_template.replace("{+asset_path}", "")

                try:
                    extracted_text = requests.get("GET", url_template, headers=box_client.headers)
                    requests.raise_for_status()
                except requests.RequestException as e:
                    print(f"Error downloading extracted text for file {file['id']}: {e}")
                    return BoxClientError(f"Failed to download extracted text for file {file['id']}")
